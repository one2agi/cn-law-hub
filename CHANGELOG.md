...
## v2.1.0 — 诉讼实务增强与检索性能大幅提速 (2026-09-25)

### 新增：诉讼律师实务增强
- **纯 Python MS-DOC (Word 97-2004) 原生解析**：基于 `olefile` 实现原生 FIB 与 Piece Table 二进制流解码器，彻底摆脱 `antiword` / `catdoc` 等外部系统命令依赖，稳定提取最高法早期司法解释正文。
- **公文类型分类与《修改决定》条号顺移预警**：自动识别《修改决定》与《修正案》打标 `amendment_decision`，主动提示条号顺移风险，防范文书条号引用失误。
- **境内政务网代理抑制与自愈直连**：针对 `.gov.cn`、`.12371.cn` 自动抑制本地代理（含 SOCKS5 `ALL_PROXY`），并在 `ProxyError` 时瞬时回退直连。
- **裁判文书标准引用格式化工具 (`format_legal_citation`)**：依最高法【法释〔2009〕14号】规范，将输入条号、款、项及刑法分条（如“第一百三十三条之一”）一键转为规范中文引用。

### 优化：检索性能提速 3~4 倍
- **全局 HTTP 连接池复用**：长连接复用覆盖全部 10 个数据源，减少重复握手延迟。
- **8 秒自愈超时**：替代原 30 秒超时，支持 `NPC_LAW_TIMEOUT` 环境变量调优。
- **多路轻量并发**：最高法发布库 3 线程并发抓取（耗时从 16s 降至 3.7s）；跨法律法条检索并发下载（耗时从 1.0s 降至 0.3s）。
- **断点续查缓存隔离**：修复 `--resume` 模式下并发预下载导致的缓存误杀漏检。

### 测试
新增 24 个离线与集成测试，覆盖 doc 解析、公文分类、网络穿透、格式化与续查逻辑。
全量：**340 passed / 7 skipped**。
